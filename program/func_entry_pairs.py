...
def open_positions(client):
    ...
    # Initialize container for BotAgent result
    bot_agents = []
    ...
    for index, row in df.iterrows():
        ...
        if len(series_1) > 0 and len(series_1) == len(series_2):
            ...
            if abs(z_score) >= ZSCORE_THRESH:
                ...
                # Ensure like-for-like not already open (diversity trading)
                ...
                if not is_base_open and not is_quote_open:
                    ...
                    # Get acceptablwe price string format with correct number of decimals
                    ...
                    base_max_order_size = markets["markets"][base_market]["maxPositionSize"]
                    quote_max_order_size = markets["markets"][quote_market]["maxPositionSize"]
                    check_base = float(base_max_order_size) > float(base_quantity) > float(base_min_order_size)
                    check_quote = float(quote_max_order_size) > float(quote_quantity) > float(quote_min_order_size)
    ...
    # Save agents
    print(f"Success: Manage open trade checked. Number of open trades: {len(bot_agents)}")
    if len(bot_agents) > 0:
